...
+ (NSMutableArray *)filterTriggerDevices:(NSArray *)devices {
    ...
    for (DeviceModel *deviceModel in devices) {
        ...
        } else if (![deviceModel.accessMode isEqualToString:@"LoRa"] && [self filterTriggerDatapoints:[IntoYunFMDBTool getDatapointListArray:deviceModel.pidImp]].count > 0) {
            [filter addObject:deviceModel];
        }
    }
    return filter;
...
+ (NSMutableArray *)filterActionDevices:(NSArray *)devices {
    ...
    for (DeviceModel *deviceModel in devices) {
        if ([deviceModel.deviceId isEqualToString:SYSTEM_DEVICE_ID]) {
            [filter addObject:deviceModel];
        } else if (![deviceModel.accessMode isEqualToString:@"LoRa"] && [self filterActionDatapoints:[IntoYunFMDBTool getDatapointListArray:deviceModel.pidImp]].count > 0) {
            [filter addObject:deviceModel];
        }
    }
    return filter;
